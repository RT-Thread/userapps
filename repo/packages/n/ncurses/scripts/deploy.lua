...
-- Date           Author       Notes
-- ------------   ----------   -----------------------------------------------
-- 2023-06-26     zy           copy terminfo database
...
function main(rootfs, installdir)
    ...
    for _, filedir in ipairs(os.filedirs(path.join(installdir, "share", "terminfo") .. "/*")) do
        local inc = path.join(installdir, "share", "terminfo")
        local name = path.relative(filedir, inc)
        rt_utils.cp_with_symlink(path.join(inc, name), path.join(rootfs, "etc", "terminfo", name),
