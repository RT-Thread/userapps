...
function main(rootfs, installdir)
    for _, filedir in ipairs(os.filedirs(path.join(installdir, "include", "SDL2") .. "/*")) do
        local inc = path.join(installdir, "include", "SDL2")
        local name = path.relative(filedir, inc)
        rt_utils.cp_with_symlink(path.join(inc, name), path.join(rootfs, "include", "SDL2", name),
